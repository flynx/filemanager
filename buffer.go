...
func (this *LinesBuffer) String() string {
	...
	//	if ! line.Populated {
	//		continue }
	for line := range this.Iter(IterBlank) {
...
// NOTE: this can't know if a command has skipped a value or is still 
//		thinking about it, so it is recommended to return empty strings 
//		instead of skipped values, e.g:
//			$ ls \
//				| go run lines \
//					-m "sed 's/\(.*\.go.*\)\|.*/\1/'" \
//					-m "sed 's/$/| <size> /'"
//		instead of the traditional grep:
//			$ ls \
//				| go run lines \
//					-m "grep '\.go'" \
//					-m "sed 's/$/| <size> /'"
//		but if it is not needed to draw intermediate results then let the 
//		shell take care of it:
//			$ go run lines \
//				-c "ls | grep '\.go'" | sed 's/$/| <size> /'"
// NOTE: if one -m handler returns an empty string for a value, the next 
//		handler will not see either the value nor the empty string...
//
